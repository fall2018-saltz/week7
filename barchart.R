...
murder_bar_chart<-ggplot(murders_per_state,aes(x=state, y=murders_each_state)) +
  geom_col()
murder_bar_chart

# Create a bar chart with the number of murders per state on the adjusted axis
murder_bar_chart2<-ggplot(murders_per_state,aes(x=state, y=murders_each_state)) +
  geom_col() + 
...
murder_data<-murders_per_state[order(murders_per_state$murders_each_state, decreasing= TRUE),]
#create rows at 1
rownames(murder_data)<-NULL
#Create the bar graph
murder_bar_chart3<-ggplot(murder_data,aes(x=reorder(state,-murders_each_state), y=murders_each_state)) +
...
#bar graph
murder_bar_chart4<-ggplot(murder_data,aes(x=reorder(state,-murders_each_state), y=murders_each_state, group= 1, fill=popOver18)) +
  geom_bar(stat = "identity") + 
  ggtitle("Total Murders") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
murder_bar_chart4
